...
{	
	if (CH[channel].wave_count <= 2)/*   维持200us的固定高电平时间，需要定时器进入两次，两次之内电平必须为高  */
	{	
...
{
  if(CH[ch_num].get_new_data)   /*  校验接收到新数据，置低对应通道的波形计数值以实时改变或切换波形 */
	{
		CH[ch_num].get_new_data = FALSE;

		if (rep_count < 1)  //如果重复次数小于1，表示接收到的数据是新的不同的数据，则需对对应通道计数清零
		{
			CH[ch_num].wave_count = 0;
      /***************如果接收到正确数据，则初始化断续、疏密波的标志位，使其重新开始*****************/
    	CH[ch_num].Iwave_count_flag = FALSE;
    	CH[ch_num].Dwave_count_flag = FALSE;
    	/***************如果接收到正确数据，则初始化断续、疏密波的时间计数，使其重新开始***************/
    	CH[ch_num].time5s_count = 0;
    }
	}
}
/* Private functions ---------------------------------------------------------*/
